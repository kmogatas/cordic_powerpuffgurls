...
void cordiccart2pol(input_t x, input_t y, radius_t * r,  theta_t * theta)
{
    ...
    for (int i = 0; i < 20; i++) {
        ...
        input_t x_shift = (sigma == 1) ? (input_t)(current_y >> i) : (input_t)(- (current_y >> i));
        input_t y_shift = (sigma == 1) ? (input_t)(current_x >> i) : (input_t)(- (current_x >> i));

        current_x = current_x - x_shift;
        current_y = current_y + y_shift;

        theta_base = (sigma == 1) ? theta_base - angles[i] : theta_base + angles[i];
    }
